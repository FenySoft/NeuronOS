...
**Neuron OS** — capability-based actor runtime for .NET, co-designed with the Cognitive Fabric Processing Unit (CFPU). Today a reference runtime on any .NET host; tomorrow runs natively on CFPU silicon where each core is physically an actor with private SRAM and hardware mailbox FIFOs. Apache-2.0.
...
1. **`IMailbox` / `TMailbox`** (`src/NeuronOS.Core/IMailbox.cs`, `TMailbox.cs`)
...
2. **`TActorRef`** (`src/NeuronOS.Core/TActorRef.cs`)
...
3. **`TActor<TState>` + `TActorSystem`** (`src/NeuronOS.Core/TActor.cs`, `TActorSystem.cs`)
...
```
src/NeuronOS.Core/              runtime (only project today)
tests/NeuronOS.Core.Tests/      xUnit, mirrors src/ structure
...
`CONTRIBUTING.md` mandates test-first. Concretely this means: **do not add/modify runtime code in `src/NeuronOS.Core/` without a corresponding failing test first in `tests/NeuronOS.Core.Tests/`.** Exceptions are limited to `.csproj`, docs, workflow files. An assertion-free test counts as no test.
