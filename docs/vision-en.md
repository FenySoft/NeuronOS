...
### F2 -- Single Nano core RTL simulation
**Output:** testing the C# runtime on a Verilator-simulated single Nano core:
- Mailbox MMIO mapping (software mailbox → hardware address mapping)
- CIL execution verification at RTL level
- Actor code written in F1 runs unmodified on the simulated core
- First measurements: message-send latency, memory footprint

This is the **first hardware approximation** — the F1 software abstraction meets real RTL here.
...
The CFPU actor-fabric architecture naturally scales to problems based on **massively parallel, local interactions**. The most ambitious target areas:

- **Real-time physics simulation** -- light field computation, spatial sound propagation, material interaction, fluid dynamics. Each spatial region is an independent actor with its own state; physical interactions are modeled as neighbor-to-neighbor message passing. This approach eliminates the shared-memory synchronization bottleneck that limits centralized architectures.
- **Spatial rendering** -- the simulation fabric, paired with appropriate projector/light-field hardware, can serve as the foundation for real-time, interactive 3D spatial simulation -- the first architecturally grounded approximation of science fiction's "holodeck" concept -- where users move within and interact with the simulated environment.
  Primitive forms of the display hardware already exist today (VR/AR headsets, volumetric displays, light-field displays) -- the real bottleneck is not projection but the underlying computational model. Today's GPU shared-memory architectures do not scale to full real-time spatial simulation, which is why current VR "only" projects rendered images rather than simulating real physics at every point. The CFPU actor-fabric provides exactly this missing layer: simulation and rendering are decoupled, making it combinable with any future display technology.
