...
> **📦 Implementáció:** [`src/NeuronOS.Core`](../src/NeuronOS.Core) — a tényleges runtime kód, tesztek és API a `src/` és `tests/` mappákban van. Ez a dokumentum a hosszú távú **víziót** rögzíti.
...
> *A Neuron OS a **CFPU** hardveres platformra készül — a CFPU referencia implementációja a **CLI-CPU** projekt ([github.com/FenySoft/CLI-CPU](https://github.com/FenySoft/CLI-CPU)). A CFPU ↔ CLI-CPU kapcsolat részletei: [CLI-CPU FAQ #1](https://github.com/FenySoft/CLI-CPU/blob/main/docs/faq-hu.md#1-mi-a-cfpu-és-mi-a-kapcsolata-a-cli-cpu-val).*
...
### F2 — Single Nano core RTL szimuláció
**Kimenet:** a C# runtime tesztelése a Verilator-szimulált egyetlen Nano core-on:
- Mailbox MMIO mapping (szoftveres mailbox → hardveres cím-térképezés)
- CIL futtatás igazolása RTL szinten
- Az F1-ben írt aktor kód változtatás nélkül fut a szimulált core-on
- Első mérések: üzenetküldési latencia, memória-lábnyom

Ez az **első hardver-közelítés** — az F1 szoftveres absztrakció itt találkozik a valódi RTL-lel.
...
### Távlati alkalmazások — F7 után

A CFPU aktor-fabric architektúrája természetes módon skálázódik olyan problémákra, amelyek **masszívan párhuzamos, lokális kölcsönhatásokon** alapulnak. A legambiciózusabb célterületek:

- **Valós idejű fizikai szimuláció** — fénymező-számítás, térbeli hangterjedés, anyag-kölcsönhatás, folyadékdinamika. Minden térrész önálló aktor saját állapottal; a fizikai kölcsönhatások szomszédsági üzenetváltásként modellezhetők. Ez a megközelítés eliminálja a hagyományos shared-memory szinkronizációs bottlenecket, ami a centralizált architektúrákat korlátozza.
- **Térbeli megjelenítés** — a szimulációs fabric megfelelő kivetítő/fénymező hardverrel párosítva valós idejű, interaktív 3D térszimuláció alapja lehet — a science fiction „holodeck" koncepciójának első architekturálisan megalapozott közelítése —, ahol a felhasználó a szimulált térben mozog és hat a környezetére.
  A megjelenítő hardver primitív formái már ma léteznek (VR/AR headset-ek, volumetrikus kijelzők, fénymező display-ek) — a valódi szűk keresztmetszet nem a kivetítés, hanem a mögöttes számítási modell. A mai GPU-k shared-memory architektúrája nem skálázódik a valós idejű, teljes térszimuláció szintjére, ezért a jelenlegi VR „csak" renderelt képet vetít, nem szimulál valódi fizikát minden pontban. A CFPU aktor-fabric pont ezt a hiányzó réteget adja: a szimuláció és a megjelenítés szétválik, így bármilyen jövőbeli kivetítő technológiával kombinálható.

- **Spiking Neural Network (SNN)** — a biológiai ideghálózat természetes leképezése: minden neuron egy aktor, a spike-ok üzenetek, az aktor referenciák (`TActorRef`) pedig a szinaptikus kapcsolatok — aki rendelkezik a referenciával, az küldhet spike-ot, pont mint a biológiában. A CFPU hardveres mailbox-ok a szinaptikus átvitelt modellezik natívan, szoftver-overhead nélkül.

Ezek nem ígéretek, hanem **architekturális motiváció**: a CFPU + Neuron OS tervezési döntéseinek igazolása, hogy az aktor-alapú, shared-nothing modell nem csupán elméleti elegancia, hanem a jövő számítási problémáinak természetes nyelve.
...
4. **Dokumentum frissítések**: ahogy a valós munka halad, ez a `vision-hu.md` dokumentum **frissül**, és a nyitott kérdések **eldőlnek**.
...
| 1.3 | 2026-04-17 | **Átköltözés a Neuron OS repóba** (korábban `CLI-CPU/docs/neuron-os-hu.md`). A CLI-CPU-ban most stub marad a történelmi linkekhez. Bevezető boxok frissítve (most már ebben a repóban van az implementáció is). |
